"""
Unified metrics module for segmentation and classification evaluation.

This module merges the previous compute_metrics and classification_metrics
implementations while keeping existing callable names for compatibility.
"""
...
def compute_acc(gt: np.ndarray, pred: np.ndarray, threshold: float) -> float:
    """Compute binary accuracy after thresholding predictions."""
    pred_binary = (pred > threshold).astype(int)
    correct_cnt = (pred_binary == gt).sum()
    return correct_cnt / len(gt)


def calc_binary_classification_metrics(y_true: Sequence[int], y_pred: Sequence[int]) -> Tuple[float, float, float, float, float, float, float, float]:
    """Compute classification metrics for binary labels encoded as 0/1."""
...
def compute_seg_metrics(dct: Mapping[str, Mapping[str, Sequence[Sequence[float]]]]) -> pd.DataFrame:
    """Compute pixel-level segmentation metrics by category."""
    seg_metrics = {
        "split": [],
        "auroc": [],
        "ap": [],
    ...
    for column in seg_metrics.columns:
        if column == "split":
            new_row[column] = "Mean"
        elif pd.api.types.is_numeric_dtype(seg_metrics[column]):
...
def compute_classify_matrics(anomaly_dct: Mapping[str, Mapping[str, Sequence[int]]]) -> pd.DataFrame:
    """Compute category-wise binary classification metrics."""
    anomaly_metrics = dict(split=[], Acc=[], F1=[], Precision=[], Recall=[], AUROC=[], AUPR=[])
    for k, v in anomaly_dct.items():
        acc, f1_score, precision, recall, _, _, auroc, aupr = calc_binary_classification_metrics(v["true"], v["pred"])
    ...
    for column in anomaly_score.columns:
        if column == "split":
            new_row[column] = "Mean"
        elif pd.api.types.is_numeric_dtype(anomaly_score[column]):
